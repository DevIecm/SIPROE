    ...
    try {
        ...
        const result = await pool.request()
            .query(`SELECT *
                    ...
                    proyectos p
                    INNER JOIN
                    unidad_territorial u ON p.ut = u.clave_ut
                    GROUP BY
                    u.distrito
                    ...
                    unidad_territorial u ON p.ut = u.clave_ut
                    LEFT JOIN
                    sorteo s ON s.clave_ut = u.clave_ut
                    ) AS resultado
                    ORDER BY
                    CASE
                    WHEN distrito = 'TOTAL' THEN 1
                    ELSE 0
                    END,
                    CASE
                    WHEN distrito = 'TOTAL' THEN NULL
                    ELSE CAST(distrito AS INT)
                    END;`)
        ...
        return res.status(500).json({ message: "Error de servidor", error: error.stack});
    }
});
